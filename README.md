# 👋 Hi, I'm Sumit

### Backend-Focused Software Engineer | Java • Spring Boot • AI

I'm a **Bachelor of Computer Applications student and backend-focused software developer** interested in building scalable, maintainable systems and AI-powered applications.

I primarily work with **Java, Spring Boot, REST APIs, databases, microservices, and Docker**, and I also build applications integrating **LLMs, RAG, and AI orchestration**.

Currently seeking an **entry-level Backend Engineer / Software Engineer** role where I can contribute to real-world products and grow as an engineer.

---

## 🚀 What I Build

- 🔹 Backend systems with **Java & Spring Boot**
- 🔹 REST APIs and service-oriented architectures
- 🔹 Microservices and distributed applications
- 🔹 Database-driven applications
- 🔹 AI-powered applications using **LLMs & RAG**
- 🔹 Containerized applications using **Docker & Kubernetes**

---

## 🛠️ Tech Stack

### 💻 Languages
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-336791?style=flat&logo=postgresql&logoColor=white)

### ⚙️ Backend
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=springboot&logoColor=white)
![Spring Security](https://img.shields.io/badge/Spring_Security-6DB33F?style=flat&logo=springsecurity&logoColor=white)
![Hibernate](https://img.shields.io/badge/Hibernate-59666C?style=flat&logo=hibernate&logoColor=white)
![Spring AI](https://img.shields.io/badge/Spring_AI-6DB33F?style=flat&logo=spring&logoColor=white)

### 🗄️ Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat&logo=redis&logoColor=white)
![Elasticsearch](https://img.shields.io/badge/Elasticsearch-005571?style=flat&logo=elasticsearch&logoColor=white)

### 🤖 AI / LLM
![RAG](https://img.shields.io/badge/RAG-412991?style=flat)
![LLM](https://img.shields.io/badge/LLM-412991?style=flat)
![Vector DB](https://img.shields.io/badge/Vector_Database-412991?style=flat)
![Prompt Engineering](https://img.shields.io/badge/Prompt_Engineering-412991?style=flat)

### ☁️ DevOps & Cloud
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![Kafka](https://img.shields.io/badge/Apache_Kafka-231F20?style=flat&logo=apachekafka&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)

### 🎨 Frontend
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)

---

# ⭐ Featured Projects

## 🤖 StackGen — AI Project Generation Platform

An AI-powered platform that generates software projects from natural-language requirements.

### Highlights

- 🤖 AI-driven project generation
- ⚙️ Spring Boot backend
- 🎨 React frontend
- 🧠 LLM orchestration using Spring AI
- 🔐 Authentication & Role-Based Access Control
- ⚡ Redis caching
- 🗄️ PostgreSQL
- 🐳 Docker
- ☸️ Kubernetes
- 📨 Kafka-based architecture
- 💳 Subscription & billing system
- 📡 Live code streaming

🔗 [View Repository](YOUR_STACKGEN_REPOSITORY)

---

## 🏥 Clinic Token Management System

A digital queue management system designed to simplify patient token generation and doctor-side queue management.

### Highlights

- 📱 QR-based patient entry
- 🎫 Automatic token generation
- 🔒 Concurrent-safe token allocation
- 👨‍⚕️ Doctor dashboard
- 🔄 Patient queue management
- 🗄️ PostgreSQL / Supabase
- ⚙️ Spring Boot REST API
- ⚛️ React frontend

🔗 [View Repository](YOUR_CLINIC_REPOSITORY)

---

## 📰 AI News Aggregator

A full-stack application that collects news and uses AI to generate concise summaries.

### Tech

`React` `Node.js` `Express` `MongoDB` `LLM Integration`

🔗 [View Repository](YOUR_NEWS_REPOSITORY)

---

# 📊 GitHub Stats

<p align="center">

<img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&hide_border=true" height="170"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&hide_border=true" height="170"/>

</p>

---

# 📈 Currently Working On

- 🧠 Improving Data Structures & Algorithms
- ⚙️ Building scalable backend systems with Spring Boot
- 🤖 Exploring AI Agents, RAG & LLM orchestration
- ☁️ Learning distributed systems and cloud infrastructure
- 🚀 Building production-oriented projects

---

# 🤝 Let's Connect

I'm open to **entry-level Backend Engineer and Software Engineer opportunities**.

<p align="left">

<a href="YOUR_LINKEDIN_URL">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:YOUR_EMAIL">
<img src="https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white"/>
</a>

</p>

---

### 💭

> Build. Break. Understand. Build better.
